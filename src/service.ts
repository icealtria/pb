...
export class PasteService {
  ...
  async createUrlPaste(
    url: string,
    ttl?: number,
  ): Promise<{ id: string; slug: string; sunset: string }> {
    let tries = 0;
    while (tries < 5) {
      try {
  ...
  async createPaste(
    ...
    if (data.label) {
      const existing = await this.db.prepare(
        "SELECT slug FROM pastes WHERE slug = ?",
      ).bind(slug).first();
      if (existing) {
  ...
  > {
    return await this.db
      .prepare(
        "SELECT id, slug, content, content_type, expires_at FROM pastes WHERE slug = ?",
      )
      .bind(slug)
      .first();
  }
  ...
  async deleteExpired(): Promise<void> {
    await this.db
      .prepare(
        "DELETE FROM pastes WHERE expires_at IS NOT NULL AND datetime(expires_at) <= datetime('now')",
      )
      .run();
  }
}
