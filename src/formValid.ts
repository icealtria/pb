...
export const formValid = async (c: Context, next: Next) => {
  ...
  const addr = url.protocol + "//" + url.hostname +
    (url.port ? ":" + url.port : "");
  c.set("addr", addr);
  ...
  if (content.length > MAX_SIZE) {
    return c.text("Content too large. Maximum size is 2MB.", 413);
  }

  c.set("content", content);
  c.set("contentType", contentType);
  if (typeof sunset === "string" && !isNaN(Number(sunset))) {
    c.set("sunset", Number(sunset));
  }
...
const parseContent = async (
  body: any,
): Promise<{ content: string | Uint8Array | null; contentType: string }> => {
  if (body instanceof File) {
    ...
    if (
      contentType === "application/octet-stream" && isTextContent(content)
    ) {
      return {
